...
This project keeps code in the repository and runtime data outside the repository. Agents should read this document and `src/common/paths.py` before reading, writing, or moving data files.
...
- **Canonical source**: `BTCUSD_*.csv` — 9 timeframes (1min / 5m / 15m / 30m / 1h / 4h / 1d / 1w / 1M).
- **Auxiliary source**: `BTCUSDT_*` — funding rate, open interest, long-short ratios. These are merged into the canonical OHLCV files by `step_indicator`.
- Indicator columns (macd, ppo, rsi, cvd, cvd_rolling, delta, ma_7/25/99, oi, funding_rate) are stored **in-place** in the CSV files by Step 2.

### processed/cycles_enriched/btc/ (canonical)

One parquet per timeframe. Each file goes through two stages:

1. **Base** (written by Step 3 — detect): cycle_id, timeframe, start_date, end_date, cycle_type, duration_candles, category, algorithm_used, candle_data (list-struct), cycle_features (nested struct).
2. **Enriched** (overwritten by Step 5 Phase 3 — context builder): base columns + relationship columns added below.
...
| parent_progress_at_start / _end | float32 | Where in parent cycle this child starts/ends |
...
| combo_4 | string | Direction combo string e.g. "UUDU" |
...
| context_meta.json | — | — | — | version, asset, data_range, timeframe_groups |

**Context columns (69 total)**:
- `timestamp`
- Per TF (×9): `{tf}_key`, `{tf}_type`, `{tf}_time_prog`, `{tf}_candle_prog`
- Per TF (×9): `{tf}_changed`, `{tf}_chg_type`
...
Historical snapshots. No active code reads from these. Do not confuse with canonical files.
...
| `src/dashboard_api/` | `cycles_enriched/btc/`, `context/btc/`, `dashboard/` | — |
| `src/services/live_update_service.py` | External APIs | `raw/market/*.csv`, triggers pipeline |
| `dashboards/chart_app/` | `/api/base-data/series` (polling) | — |
...
The report covers: raw market CSV schemas, canonical cycle parquet schemas (with nested struct types), context file structures (cycle_dim TF distribution, n_up_4 distribution, 69-col listing), dashboard payloads.
...
- The canonical cycle source is `processed/cycles_enriched/btc/cycles_*.parquet`. Do not use the root-level copies in `cycles_enriched/` (archived).
- The canonical multi-TF context source is `processed/context/btc/timeframe_context_*.parquet`.
