...
def _get_oi_lookup(tf: str) -> tuple:
    ...
    if tf in _oi_cache:
        return _oi_cache[tf]
    oi_contracts_path = PROJECT_PATHS.base_data_dir / f"BTCUSDT_oi_contracts_{tf}.csv"
    oi_legacy_path = PROJECT_PATHS.base_data_dir / f"BTCUSDT_oi_{tf}.csv"
    ohlcv_path = PROJECT_PATHS.base_data_dir / f"BTCUSD_{tf}.csv"
...
def main() -> int:
    args = parse_args()
    from src.services.live_update_service import setup_logging
