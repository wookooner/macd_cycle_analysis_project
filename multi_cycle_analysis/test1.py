...
class ImprovedFeatureVisualizer:
    ...
    def __init__(self):
        self.feature_types = {
            'integer': ['duration_candles', 'shape_core_count', 'shape_noise_count', 'shape_direction_change'],
            'ratio': ['strength_direction_pct', 'strength_hist_positive_ratio', 'strength_price_up_ratio', 
                     'strength_price_down_ratio', 'shape_peak_price_position', 'shape_trough_price_position'],
            'continuous': ['change_price_pct', 'start_rsi', 'end_rsi', 'change_rsi', 'start_macd', 'end_macd', 
                          'change_macd', 'start_hist', 'end_hist', 'change_hist', 'start_macd_signal', 
                          'end_macd_signal', 'change_macd_signal', 'volatility_max_high_pct', 'volatility_max_loss_pct',
        ...
        self.feature_names_kr = {
            'duration_candles': '지속 기간', 'shape_core_count': '핵심 캔들 수', 'shape_noise_count': '노이즈 캔들 수',
            'shape_direction_change': '방향 전환 횟수', 'strength_direction_pct': '방향성 비율',
            'strength_hist_positive_ratio': 'Hist 양수 비율', 'strength_price_up_ratio': '가격 상승 비율',
            'strength_price_down_ratio': '가격 하락 비율', 'shape_peak_price_position': 'Peak 위치',
            'shape_trough_price_position': 'Trough 위치', 'change_price_pct': '가격 변화율',
    ...
    def plot_integer_feature(self, ax, data_dict, feature_name, title):
        ...
        for label, data in data_dict.items():
            ...
            if len(data_series) > 0:
                cleaned_data[label] = data_series
        ...
        all_values = pd.concat([pd.Series(data) for data in cleaned_data.values()])
        ...
        try:
            if unique_count <= 30:  # 범주가 적으면 막대그래프
                ...
                # 너무 많은 unique 값이 있으면 상위 30개만
                if len(value_range) > 30:
                    value_counts = all_values.value_counts()
                    value_range = value_counts.nlargest(30).index.sort_values().tolist()
                
                bar_width = 0.8 / len(cleaned_data)
                x = np.arange(len(value_range))
                
                for i, (label, data) in enumerate(cleaned_data.items()):
                    counts = pd.Series(data).value_counts().reindex(value_range, fill_value=0)
                    offset = (i - len(cleaned_data)/2 + 0.5) * bar_width
                ...
                # x축 레이블 처리
                if len(value_range) > 15:
                    # 너무 많으면 일부만 표시
                    tick_labels = [f'{int(v)}' if i % 2 == 0 else '' for i, v in enumerate(value_range)]
                else:
                    tick_labels = [f'{int(v)}' for v in value_range]
                ...
                for i, (label, data) in enumerate(cleaned_data.items()):
                    ax.hist(data, bins=n_bins, alpha=0.6, label=label, 
            ...
            # 통계 정보
            stats_text = []
            for label, data in cleaned_data.items():
                data_series = pd.Series(data)
                mean_val = data_series.mean()
                median_val = data_series.median()
                stats_text.append(f'{label}: μ={mean_val:.1f}, M={median_val:.0f}')
    ...
    def plot_ratio_feature(self, ax, data_dict, feature_name, title):
        ...
        for label, data in data_dict.items():
            ...
            # 0-1 범위로 클리핑
            data_series = data_series.clip(0, 1)
            
            # 유효성 검증
            if data_series.std() == 0:  # 모든 값이 동일
                continue
        ...
        distribution = {}
        use_stacked_bar = True
        
        for label, data_series in cleaned_data.items():
            try:
                # pd.cut으로 구간 분류
                binned = pd.cut(data_series, bins=bins, labels=bin_labels, 
                ...
                if binned is not None:
                    ...
                    use_stacked_bar = False
                    break
                    
            except Exception as e:
                # pd.cut 실패 시 히스토그램 사용
                print(f"⚠️  {feature_name} pd.cut 실패, 히스토그램 사용: {e}")
                use_stacked_bar = False
                break
        
        # 스택 바 차트 또는 대체 시각화
        if use_stacked_bar and distribution:
            try:
                df_dist = pd.DataFrame(distribution)
                colors = ['#e74c3c', '#e67e22', '#f39c12', '#2ecc71', '#3498db']
                
                df_dist.T.plot(kind='bar', stacked=True, ax=ax, color=colors, 
                             width=0.7, legend=True)
                
                ax.set_ylabel('빈도')
                ax.set_xlabel('')
                ax.legend(title='비율 구간', bbox_to_anchor=(1.05, 1), 
                         loc='upper left', fontsize=8)
                ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right')
                
            except Exception as e:
                print(f"⚠️  스택 바 차트 생성 실패: {e}")
                use_stacked_bar = False
        
        # 스택 바 실패 시 단순 히스토그램
        if not use_stacked_bar:
            colors = ['#ff6b6b', '#4ecdc4', '#95e1d3', '#f38181']
            for i, (label, data_series) in enumerate(cleaned_data.items()):
                ax.hist(data_series, bins=20, alpha=0.6, label=label,
                       color=colors[i % len(colors)], edgecolor='black')
            
            ax.set_xlabel('비율')
            ax.set_ylabel('빈도')
            ax.legend()
                #distribution[label] = pd.Series(hist, index=bin_labels)
        
        if not distribution:
            ax.text(0.5, 0.5, '분포 계산 실패', ha='center', va='center')
            ax.set_title(title)
            return
        ...
        try:
            df_dist.T.plot(kind='bar', stacked=True, ax=ax, color=colors, width=0.7, legend=True)
        except Exception as e:
            # plot 실패 시 단순 막대그래프
            for i, (label, data_series) in enumerate(cleaned_data.items()):
                ax.bar(i, len(data_series), alpha=0.7, label=label, color=colors[i % len(colors)])
            ax.set_xticks(range(len(cleaned_data)))
            ax.set_xticklabels(cleaned_data.keys(), rotation=45, ha='right')
            ax.legend()
        ...
        for label, data_series in cleaned_data.items():
            mean_val = data_series.mean()
            median_val = data_series.median()
            stats_text.append(f'{label}: μ={mean_val:.2%}, M={median_val:.2%}')
        
        # 데이터 범위 변환 정보 추가
        if data_range_info:
            stats_text.append('---')
            stats_text.extend(data_range_info)
        
        stats_str = '\n'.join(stats_text)
        try:
            ax.text(0.02, 0.98, stats_str, transform=ax.transAxes, fontsize=7,
    ...
    def plot_continuous_feature(self, ax, data_dict, feature_name, title):
        ...
        for label, data in zip(labels, data_list):
            ...
            # 값의 크기에 따라 포맷 조정
            if abs(mean_val) < 0.01 and abs(mean_val) > 0:
                stats_text.append(f'{label}: μ={mean_val:.4f}, M={median_val:.4f}')
            elif abs(mean_val) > 1000:
                stats_text.append(f'{label}: μ={mean_val:.0f}, M={median_val:.0f}')
            else:
                stats_text.append(f'{label}: μ={mean_val:.2f}, M={median_val:.2f}')
    ...
    def visualize_all_features(self, feature_df, timeframe, output_path=None, title_prefix="", 
        ...
        Args:
            feature_df: 특징 데이터프레임
            timeframe: 시간대
            output_path: 저장 경로 (카테고리별 분리 시 자동으로 _category 접미사 추가)
            title_prefix: 제목 접두사
            separate_by_category: True면 카테고리별로 분리된 파일 생성
        """
        ...
        # 특징을 카테고리별로 분류
        feature_categories = {
            'Shape (형태)': ['duration_candles', 'shape_core_count', 'shape_noise_count', 
                           'shape_direction_change', 'shape_peak_price_position', 'shape_trough_price_position'],
            'Strength (강도)': ['strength_direction_pct', 'strength_hist_positive_ratio', 
                              'strength_price_up_ratio', 'strength_price_down_ratio'],
            'Start (시작)': ['start_rsi', 'start_macd', 'start_hist', 'start_macd_signal'],
            'End (종료)': ['end_rsi', 'end_macd', 'end_hist', 'end_macd_signal'],
            'Change (변화)': ['change_price_pct', 'change_rsi', 'change_macd', 
                            'change_hist', 'change_macd_signal'],
            'Volatility (변동성)': ['volatility_max_high_pct', 'volatility_max_loss_pct',
                                  'volatility_max_intraday_high_pct', 'volatility_max_intraday_loss_pct',
                                  'volatility_avg_true_range', 'volatility_price_change_deviation']
        }
        ...
        if separate_by_category:
            ...
            for category_name, features in feature_categories.items():
                ...
                self._create_category_plot(
                    feature_df, up_cycles, down_cycles, available_features,
                    category_name, timeframe, title_prefix, output_path
                )
        else:
            # 기존 방식: 한 화면에 모두
            all_features = (self.feature_types['integer'] + self.feature_types['ratio'] + 
            ...
            self._create_single_plot(
                feature_df, up_cycles, down_cycles, available_features,
                timeframe, title_prefix, output_path
            )
    
    def _create_category_plot(self, feature_df, up_cycles, down_cycles, features, 
                             category_name, timeframe, title_prefix, output_path):
        """카테고리별 플롯 생성"""
        n_features = len(features)
        n_cols = 3  # 3열로 조정 (더 크게 보이도록)
        n_rows = (n_features + n_cols - 1) // n_cols
        
        fig = plt.figure(figsize=(18, 6 * n_rows))  # 높이 증가
        title_text = f'{title_prefix} {timeframe.upper()} - {category_name} (총 {len(feature_df)}개: 상승 {len(up_cycles)}, 하락 {len(down_cycles)})'
        ...
        for idx, col in enumerate(features, 1):
            ...
            chart_title = f'{feature_name_kr}\n({col})'
            feature_type = self.get_feature_type(col)
            
            if feature_type == 'integer':
                self.plot_integer_feature(ax, data_dict, col, chart_title)
            elif feature_type == 'ratio':
                self.plot_ratio_feature(ax, data_dict, col, chart_title)
            else:
                self.plot_continuous_feature(ax, data_dict, col, chart_title)
        ...
        if output_path:
            ...
            category_safe = category_name.split('(')[0].strip().replace(' ', '_')
            new_filename = f"{base_name}_{category_safe}{extension}"
            new_path = output_dir / new_filename
            
            plt.savefig(new_path, dpi=150, bbox_inches='tight')
            print(f"✅ {category_name} 시각화 저장: {new_path}")
        
        plt.show()
        plt.close()
    
    def _create_single_plot(self, feature_df, up_cycles, down_cycles, features,
                           timeframe, title_prefix, output_path):
        """단일 플롯 생성 (기존 방식)"""
        n_features = len(features)
        n_cols = 4
        ...
        title_text = f'{title_prefix} {timeframe.upper()} 특징 분포 (총 {len(feature_df)}개: 상승 {len(up_cycles)}, 하락 {len(down_cycles)})'
        fig.suptitle(title_text, fontsize=16, fontweight='bold', y=0.998)
...
class ImprovedHierarchicalCycleAnalyzer:
    ...
    def visualize_cycle_features(self, df, timeframe, label="사이클"):
        ...
        # 시각화 방식 선택
        print("\n시각화 방식을 선택하세요:")
        print("  1. 카테고리별 분리 (권장) - 보기 편함")
        print("  2. 전체 한번에 - 한 파일에 모두")
        
        viz_choice = input("선택 (1-2, Enter=1): ").strip() or "1"
        
        separate = viz_choice == "1"
