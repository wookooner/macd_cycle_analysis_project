import EChartPanel from "../echarts/EChartPanel.jsx";
import { withCartesianInteractions } from "../echarts/interactiveOptions.js";

function groupRows(rows, colorField) {
  if (!colorField) {
    return [{ name: "series", rows }];
  }

  const groups = new Map();
  rows.forEach((row) => {
    const key = String(row[colorField] ?? "unknown");
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(row);
  });
  return [...groups.entries()].map(([name, entries]) => ({ name, rows: entries }));
}

export default function ScatterChart({ chart }) {
  const rows = chart.rows ?? [];
  if (!rows.length) {
    return <div className="chart-empty">No scatter data available.</div>;
  }

  const datetimeAxis = chart.x_type === "datetime";
  const seriesGroups = groupRows(rows, chart.color);
  const option = withCartesianInteractions({
    animationDuration: 200,
    tooltip: {
      trigger: "item",
      formatter: (params) => {
        const data = params.data;
        return `${params.seriesName}<br/>${chart.x}: ${data[0]}<br/>${chart.y}: ${data[1]}<br/>count: ${data[2]}`;
      },
    },
    grid: { left: 60, right: 28, top: 36, bottom: 48 },
    legend: chart.color ? { top: 6 } : undefined,
    xAxis: {
      type: datetimeAxis ? "time" : "value",
      name: chart.x,
      axisLine: { onZero: !datetimeAxis },
    },
    yAxis: {
      type: "value",
      name: chart.y,
      axisLine: { onZero: true },
    },
    series: seriesGroups.map((group) => ({
      name: group.name,
      type: "scatter",
      large: true,
      largeThreshold: 2000,
      symbolSize: (value) => Math.min(16, 6 + Math.log2((value?.[2] ?? 1) + 1) * 2),
      data: group.rows.map((row) => [row[chart.x], Number(row[chart.y] ?? 0), Number(row.count ?? 1)]),
    })),
    graphic: chart.downsampled
      ? [
          {
            type: "text",
            right: 18,
            top: 12,
            style: {
              text: `binned ${chart.source_row_count} rows into ${chart.rendered_row_count} points`,
              fill: "#8a755f",
              fontSize: 11,
            },
          },
        ]
      : undefined,
  }, { zoom: true, brush: true });

  return <EChartPanel option={option} height={360} />;
}
