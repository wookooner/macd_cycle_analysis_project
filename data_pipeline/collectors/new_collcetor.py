...
LS_RATIO_LIMIT     = 500   # 탑 트레이더 롱숏 비율 API 최대 limit

# 탑 트레이더 롱숏 비율 수집 타임프레임
# period 파라미터: 5m/15m/30m/1h/2h/4h/6h/12h/1d
LS_RATIO_PERIOD_MAP = {
    "1h": "1h",
    "4h": "4h",
    "1d": "1d",
...
FUTURES_DATA_FILES = {
    ...
    # 탑 트레이더 포지션 롱숏 비율 (계약 수 기준)
    "ls_pos_ratio_1h":   "BTCUSDT_ls_pos_ratio_1h.csv",
    "ls_pos_ratio_4h":   "BTCUSDT_ls_pos_ratio_4h.csv",
    "ls_pos_ratio_1d":   "BTCUSDT_ls_pos_ratio_1d.csv",
    # 탑 트레이더 계정 롱숏 비율 (계정 수 기준)
    "ls_acc_ratio_1h":   "BTCUSDT_ls_acc_ratio_1h.csv",
    "ls_acc_ratio_4h":   "BTCUSDT_ls_acc_ratio_4h.csv",
    "ls_acc_ratio_1d":   "BTCUSDT_ls_acc_ratio_1d.csv",
}
...
class AdvancedBTCDataCollectorV2:
    ...
    def _fetch_top_trader_ls_position_ratio(
        self, period: str, start_dt: datetime, end_dt: datetime
    ) -> pd.DataFrame | None:
        """
        바이낸스 선물 탑 트레이더 포지션 롱숏 비율 수집.
        엔드포인트: GET /futures/data/topLongShortPositionRatio
        - 계약 수(포지션) 기준으로 상위 트레이더의 롱/숏 비율 계산
        - longShortRatio: 롱 비율 / 숏 비율 (>1 = 롱 우세)
        - longAccount: 롱 포지션 비율 (0~1)
        - shortAccount: 숏 포지션 비율 (0~1)

        사이클 분석 활용:
        - 롱숏 비율 극단값 = 역방향 신호 (과열 지표)
        - 사이클 시작점의 롱숏 포지션 쏠림 정도 파악
        """
        if self.futures_client is None:
            self.logger.error("Futures 클라이언트가 없어 탑 트레이더 포지션 롱숏 비율 수집 불가")
            return None

        start_ms = int(start_dt.timestamp() * 1000)
        end_ms   = int(end_dt.timestamp() * 1000)

        self.logger.info(f"[탑 트레이더 포지션 롱숏 비율 {period}] 수집 시작: {start_dt} ~ {end_dt}")
        all_rows = []
        fetch_start = start_ms

        while True:
            try:
                data = self.futures_client.top_long_short_position_ratio(
                    symbol=FUTURES_SYMBOL,
                    period=period,
                    startTime=fetch_start,
                    endTime=end_ms,
                    limit=LS_RATIO_LIMIT
                )
                if not data:
                    break
                ...
                if len(data) < LS_RATIO_LIMIT:
                    break
                time.sleep(REQUEST_DELAY)

            except ClientError as e:
                self.logger.error(f"탑 트레이더 포지션 롱숏 비율 API 오류: {e.status_code} - {e.error_message}")
                return None
            except Exception as e:
                self.logger.error(f"탑 트레이더 포지션 롱숏 비율 수집 오류: {e}")
                return None
        ...
        df['long_position']   = pd.to_numeric(df['longAccount'],    errors='coerce')
        df['short_position']  = pd.to_numeric(df['shortAccount'],   errors='coerce')
        df['symbol']          = FUTURES_SYMBOL
        df['period']          = period

        result = df[['unix', 'date', 'symbol', 'period',
                     'ls_ratio', 'long_position', 'short_position']].copy()
        result = result.sort_values('unix').drop_duplicates(subset=['unix'])
        result['ls_ratio']       = result['ls_ratio'].round(4)
        result['long_position']  = result['long_position'].round(4)
        result['short_position'] = result['short_position'].round(4)

        self.logger.info(f"탑 트레이더 포지션 롱숏 비율 {period} 포맷 완료: {len(result)}행")
        return result

    def _fetch_top_trader_ls_account_ratio(
        self, period: str, start_dt: datetime, end_dt: datetime
    ) -> pd.DataFrame | None:
        """
        바이낸스 선물 탑 트레이더 계정 롱숏 비율 수집.
        엔드포인트: GET /futures/data/topLongShortAccountRatio
        - 계정 수 기준으로 상위 트레이더의 롱/숏 비율 계산
        - 포지션 비율과 달리 한 계정이 여러 포지션을 보유해도 1로 카운트
        - 두 지표 비교: 계정 비율 > 포지션 비율 → 소수 대형 트레이더가 반대 방향
        """
        if self.futures_client is None:
            self.logger.error("Futures 클라이언트가 없어 탑 트레이더 계정 롱숏 비율 수집 불가")
            return None

        start_ms = int(start_dt.timestamp() * 1000)
        end_ms   = int(end_dt.timestamp() * 1000)

        self.logger.info(f"[탑 트레이더 계정 롱숏 비율 {period}] 수집 시작: {start_dt} ~ {end_dt}")
        all_rows = []
        fetch_start = start_ms

        while True:
            try:
                data = self.futures_client.top_long_short_account_ratio(
                    symbol=FUTURES_SYMBOL,
                    period=period,
                    startTime=fetch_start,
                    endTime=end_ms,
                ...
                self.logger.error(f"탑 트레이더 계정 롱숏 비율 API 오류: {e.status_code} - {e.error_message}")
                return None
            except Exception as e:
                self.logger.error(f"탑 트레이더 계정 롱숏 비율 수집 오류: {e}")
                return None
        ...
        result = df[['unix', 'date', 'symbol', 'period',
                     'ls_ratio', 'long_account', 'short_account']].copy()
        result = result.sort_values('unix').drop_duplicates(subset=['unix'])
        result['ls_ratio']      = result['ls_ratio'].round(4)
        result['long_account']  = result['long_account'].round(4)
        result['short_account'] = result['short_account'].round(4)

        self.logger.info(f"탑 트레이더 계정 롱숏 비율 {period} 포맷 완료: {len(result)}행")
        return result
    ...
    def update_top_trader_ls_ratio(self, period: str = "4h", start_dt: datetime = None):
        """
        탑 트레이더 롱숏 비율(포지션 + 계정) 업데이트.
        주의: 바이낸스 히스토리 제공 기간 제한 있음 (30일 내외).
              정기적으로 수집해서 누적 저장 필요.
        """
        if period not in LS_RATIO_PERIOD_MAP:
            self.logger.error(f"지원하지 않는 period: {period}. 사용 가능: {list(LS_RATIO_PERIOD_MAP.keys())}")
            return

        from datetime import timedelta

        # ── 포지션 비율 ──────────────────────────────────────────────────────
        self.logger.info(f"===== 탑 트레이더 포지션 롱숏 비율 {period} 업데이트 =====")
        pos_file_key  = f"ls_pos_ratio_{period}"
        pos_file_path = RAW_DATA_DIR / FUTURES_DATA_FILES[pos_file_key]
        pos_existing  = self._load_existing(pos_file_path)

        if not pos_existing.empty:
            last_unix   = pos_existing['unix'].iloc[-1]
            pos_start   = datetime.fromtimestamp(last_unix, tz=timezone.utc)
        elif start_dt is not None:
            pos_start   = start_dt
        else:
            pos_start   = datetime.now(timezone.utc) - timedelta(days=30)

        pos_new = self._fetch_top_trader_ls_position_ratio(period, pos_start, datetime.now(timezone.utc))
        self._merge_and_save(pos_existing, pos_new, pos_file_path)

        # ── 계정 비율 ────────────────────────────────────────────────────────
        self.logger.info(f"===== 탑 트레이더 계정 롱숏 비율 {period} 업데이트 =====")
        acc_file_key  = f"ls_acc_ratio_{period}"
        acc_file_path = RAW_DATA_DIR / FUTURES_DATA_FILES[acc_file_key]
        acc_existing  = self._load_existing(acc_file_path)

        if not acc_existing.empty:
            last_unix   = acc_existing['unix'].iloc[-1]
            acc_start   = datetime.fromtimestamp(last_unix, tz=timezone.utc)
        elif start_dt is not None:
            acc_start   = start_dt
        else:
            acc_start   = datetime.now(timezone.utc) - timedelta(days=30)

        acc_new = self._fetch_top_trader_ls_account_ratio(period, acc_start, datetime.now(timezone.utc))
        self._merge_and_save(acc_existing, acc_new, acc_file_path)
    ...
    def update_all_futures_data(self):
        """펀딩비 + OI + 탑 트레이더 롱숏 비율 일괄 업데이트"""
        self.logger.info("🚀 선물 데이터(펀딩비 + OI + 롱숏 비율) 업데이트 시작")
        self.update_funding_rate()
        for period in OI_PERIOD_MAP.keys():
            self.update_oi(period)
        for period in LS_RATIO_PERIOD_MAP.keys():
            self.update_top_trader_ls_ratio(period)
...
if __name__ == "__main__":
    ...
    print("5. 탑 트레이더 롱숏 비율 업데이트")
    print("6. 선물 데이터 전체 업데이트 (펀딩비 + OI + 롱숏 비율)")
    print("7. 전체 업데이트 (OHLCV + 선물 데이터)")

    choice = input("\n원하는 작업을 선택하세요 (1~7): ").strip()
    ...
    if choice == "1":
        ...
        print(f"사용 가능한 period: {list(LS_RATIO_PERIOD_MAP.keys())}")
        period = input("period 입력 (기본값 4h): ").strip() or "4h"
        collector.update_top_trader_ls_ratio(period)
